...
Ваш друг, чиновник Сигизмунд любитель коллекционировать билеты. Друзья Сигизмунда, зная о его хобби, часто пополняют коллекцию друга билетами банка России. Однажды Сигизмунд узнал, что ему принесли билеты принадлежащие полковнику полиции, начальнику управления экономической безопасности и противодействия коррупции Посадину.  
Чтобы Посадин не отобрал всю коллекцию Сигизмунда, напишите программу, которая выберет из списка билетов Сигизмунда все принадлежащее Посадину, если известно, что:

- Номер каждого билета состоит из двух строчных латинских букв $[a-z]$ и семи цифр $[0-9]$.
- Билеты Посандина начинаются на букву $a$ и заканчиваются на 55661, т.е соответствуют шаблону: `a???55661`.

Номера найденных билетов Посадина выведите на экран.
...
**Формат вывода**  
В одной строке через пробел выведите номера билетов соответствующих шаблону: `a???55661`, если `?` означает 1 любой символ. Если таких не окажется выведите `-1`. При проверке шаблон изменяться НЕ будет.
...
Ваш друг, медвежатник Сигизмунд частенько забывает пароль от сейфа. Он просит вас помочь ему с этой проблемой и написать программу, которая на вход принимает $n$ - количество символов в пароле и строку состоящую из $k$ символов.  
Известно, что:

- символы - это строчные латинские буквы $[a-z]$;
- если $k<n$, некоторые из символов используются в пароле несколько раз.

По этим данным нужно сгенерировать и вывести на экран все возможные комбинации пароля по $n$ символов БЕЗ повторов.

**Формат ввода**  
В первой строке входных данных передаётся целое положительное число $n\in[1...8]$. Во второй строке передаётся последовательность из $k\leq n$ строчные латинские буквы $[a-z]$.
...
Ваш друг, алкоголик Сигизмунд просит вас помочь с проблемой. Как истинный ценитель спиртного Сигизмунд хочет выпить его как можно больше, но активы Сигизмунда очень ограничены, а именно у него есть:

- Сумма в $n$ рублей;
- $k$ пустых бутылок, которые можно обменять на рубли. Бутылки отличаются по стоимости;

У Сигизмунда есть в наличии $k$ бутылок от различных напитков, которые он может сдать и получить деньги. За полученные деньги Сигизмунд может купить новую бутылку со спиртным.  
Напишите, прграмму, которая позволяет максимизировать количество спиртного выпитого Сигизмундом.
