...
Каждая картинка обязательно снабжается подрисуночной подписью, в формате: *Рис. X. Что изображено на картине*. Несколько рисунков подряд размещать запрещено, в этом случае они должны быть разделены, хотя бы одним абзацем текста. Файлы-картинки, используемые в отчёте размещаются в том же каталоге, что и сам отчёт в каталоге "image". Использовать сторонние ресурсы для хранения изображений запрещено.
