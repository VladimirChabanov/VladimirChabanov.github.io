...
Разработать сервис предоставляющий данные о погоде в городе Симферополе на момент запроса.  В качестве источника данных о погоде используйте сервис: http://openweathermap.org/. В состав сервиса входит: серверное приложение на языке С++ и клиентское приложение на языке Python.
...
4. Алгоритм работы серверного приложения (далее Сервера):
   ...
   3. Если приходит запрос на "/", сервер формирует и отправляет html-виджет:
      ...
      6. После того, как нужные данные найдены загружаем [шаблон виджета](./Lab1_fils/widget_template.html) и заполняем:{hourly[i].weather[0].description}, {hourly[i].weather[0].icon}, {hourly[i].temp};  
      ...
      7. Заполненный шаблон отправляется в ответ на запрос. Не забудьте поменять MIME на `text/html`.
